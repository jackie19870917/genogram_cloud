...
import com.genogram.entityvo.NewsInfoVo;
...
public class UserController {
    ...
    public Response<PersonVo> updatePersonVo(PersonVo personVo) {
        ...
        if (result) {
            return ResponseUtils.success(personVo1);
        } else {
            return ResponseUtils.error(Constants.FAILURE_CODE, null);
        }
    }
    ...
    public Response<AllUserNewsInfo> insertAllUserNewsInfo(@ApiParam("token") @RequestParam(value = "token", required = false) String token,
        ...
        return ResponseUtils.success(userNewsInfo);
    }
    ...
    public Response<NewsInfoVo> getAllUserNewsInfo(@ApiParam("主键") @RequestParam("id") Integer id) {

        AllUserNewsInfo allUserNewsInfo = allUserNewsInfoService.getAllUserNewsInfoById(id);

        AllUserLogin allUserLogin = allUserLoginService.getAllUserLoginById(allUserNewsInfo.getUserId());

        NewsInfoVo newsInfoVo = new NewsInfoVo();
        BeanUtils.copyProperties(allUserNewsInfo, newsInfoVo);
        newsInfoVo.setNickName(allUserLogin.getNickName());

        if (StringUtils.isEmpty(newsInfoVo)) {
            return ResponseUtils.error(Constants.FAILURE_CODE, null);
        }

        return ResponseUtils.success(newsInfoVo);
    }
    ...
    public Response<AllUserSays> insertAllUserSays(@ApiParam("token") @RequestParam(value = "token", required = false) String token,
        ...
        return ResponseUtils.success(userSays);
    }
    ...
    public Response<AllUserPics> insertOrUpdateAllUserPics(@ApiParam("token") @RequestParam(value = "token", required = false) String token,
        ...
        return ResponseUtils.success(userPics);
    }
    ...
    @ApiOperation("查询网站")
    @RequestMapping(value = "selectSysSite", method = RequestMethod.POST)
    public Response<FanSysSite> selectSysSite(@ApiParam("token") @RequestParam(value = "token", required = false) String token) {
        ...
        if (allUserLogin.getRegionCode() % 10000 == 0) {
            ...
            if (StringUtils.isEmpty(proSysSite)) {
                ...
                return ResponseUtils.success(proSysSite);
            }
